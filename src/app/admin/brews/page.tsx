...
export default function BrewsManager() {
  ...
  const handleSave = (e: React.FormEvent, data: any) => {
    e.preventDefault();
    if (editingBrew) {
      setBrews(brews.map(b => b.id === editingBrew.id ? { ...editingBrew, ...data } : b));
    } else {
      setBrews([...brews, { ...data, id: Math.random().toString(36).substr(2, 9) }]);
    }
  ...
  const filteredBrews = brews.filter(b => b.name.toLowerCase().includes(searchQuery.toLowerCase()));
        ...
        <table className="w-full text-left text-sm">
          ...
          <tbody className="divide-y divide-zinc-100">
            {filteredBrews.map((brew) => (
              <tr key={brew.id} className="hover:bg-zinc-50 transition-colors group">
                ...
                <td className="px-6 py-4">
                  <span className={cn(
                    ...
                    brew.isAvailable ? "bg-emerald-100 text-emerald-700" : "bg-red-100 text-red-700"
                  )}>
                    {brew.isAvailable ? "Available" : "Sold Out"}
                  </span>
                </div>
                <td className="px-6 py-4 text-right space-x-2">
                  <button onClick={() => { setEditingBrew(brew); setIsModalOpen(true); }} className="p-2 text-zinc-400 hover:text-primary transition-colors">
                    <Edit3 size={16} />
                  </button>
                  <button onClick={() => deleteBrew(brew.id)} className="p-2 text-zinc-400 hover:text-red-600 transition-colors">
                    <Trash2 size={16} />
                  </button>
                </div>
              </tr>
      ...
      <AnimatePresence>
        {isModalOpen && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
            ...
            >
              <h3 className="text-xl font-serif font-bold mb-6">{editingBrew ? "Edit Brew" : "Add New Brew"}</h3>
                ...
                <div className="space-y-4">
                  ...
                  <div className="flex gap-6">
                    <label className="flex items-center gap-2 cursor-pointer">
                      <input type="checkbox" name="isAvailable" defaultChecked={editingBrew?.isAvailable ?? true} className="w-4 h-4 accent-primary" />
                      <span className="text-sm text-zinc-600">Available</span>
                    </label>
                    <label className="flex items-center gap-2 cursor-pointer">
                      <input type="checkbox" name="isFeatured" defaultChecked={editingBrew?.isFeatured ?? false} className="w-4 h-4 accent-primary" />
                      <span className="text-sm text-zinc-600">Featured</span>
                    </label>
                  </div>
                </div>
                <div className="flex gap-3 pt-4">
                  <Button variant="primary" className="flex-1 rounded-none py-4">Save Brew</Button>
